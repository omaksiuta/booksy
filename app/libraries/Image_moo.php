...
class Image_moo {
    ...
    function __construct() {
        ...
        if ($this->jpeg_ignore_warnings)
            $this->ignore_jpeg_warnings();
        if ($this->can_stretch)
            $this->can_stretch(TRUE);
    }
    ...
    function get_data_stream($filename = "") {
        ...
        // ok, lets go!
        if ($filename == "")
            $filename = rand(1000, 999999) . ".jpg";     // send as jpeg
    ...
    private function _html2rgb($colour) {
        ...
        if (is_array($colour)) {
            if (count($colour) == 3)
                return $colour;        // rgb sent as an array so use it
    ...
    public function round($radius = 5, $invert = False, $corners = "") {
        ...
        // do corners
        if ($corners[0])
            imagecopymerge($this->temp_image, $corner, 0, 0, 0, 0, $radius, $radius, 100);
        $corner = imagerotate($corner, 270, 0);
        if ($corners[1])
            imagecopymerge($this->temp_image, $corner, $temp_w - $radius, 0, 0, 0, $radius, $radius, 100);
        $corner = imagerotate($corner, 270, 0);
        if ($corners[2])
            imagecopymerge($this->temp_image, $corner, $temp_w - $radius, $temp_h - $radius, 0, 0, $radius, $radius, 100);
        $corner = imagerotate($corner, 270, 0);
        if ($corners[3])
            imagecopymerge($this->temp_image, $corner, 0, $temp_h - $radius, 0, 0, $radius, $radius, 100);
