...
class Twocheckout_Charge extends Twocheckout {
    ...
    public static function form($params, $type = 'Checkout') {
        echo '<div style="text-align: center;padding:70px;" id="paymentloadingmessage" class="paymentloadingmessage"><h3> Please do not refresh the page and wait while we are processing your payment.<br>This can take a few minutes.</h3></div>';
        echo '<form style="text-align: center;" id="2checkout" action="' . Twocheckout::$baseUrl . '/checkout/purchase" method="post">';

        foreach ($params as $key => $value) {
            echo '<input type="hidden" name="' . htmlspecialchars($key, ENT_QUOTES, 'UTF-8') . '" value="' . htmlspecialchars($value, ENT_QUOTES, 'UTF-8') . '"/>';
        }
        if ($type == 'auto') {
            echo '<input style="padding:20px;border-radius:15px;color: #fff;background: #00657F;" type="submit" value="Click here if you are not redirected automatically" /></form>';
            echo '<script type="text/javascript">document.getElementById("2checkout").submit();</script>';
        } else {
            echo '<input style="padding:20px;border-radius:15px;color: #fff;background: #00657F;" type="submit" value="' . $type . '" />';
            echo '</form>';
        }
    }
